...
long long int SRManageTime = 0;
long long int MRManageTime = 0;
  ...
  while (!expt_done) {
    ...
    for (numc = 0; numc < NUMCORES; numc++) {
	  ...
	  if (nonmemops[numc]) {  /* Have some non-memory-ops to consume. */
        ...
        clock_t begin = clock();
        int read_num=get_read_num_oram_multi(addr[numc]);
        localityManageTime += clock() - begin;
        noORAMreadnum++;
        // if(read_num!=0)
